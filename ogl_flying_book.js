...
import {vertex100, fragment100, vertex300, fragment300, render_modes, textures} from "./js/ogl_constants.js";
{
    ...
    info.innerHTML = "3D object : Flying book" ;
    ...
    function shapeGenerator() {
        ...
        var skeleton = {
            ...
            children: [
                ...
                { // left wing     rotateY(sin(radians(ang)) * -20);  rect(-75, -50, 75, 100);
                    id: "leftjoint",
                    position: [-1, 0, -0],
                    scale: [0.5, 0.5, 0.5],
                    rotation: [0, 45, 0],
                ...
                { // right wing  rotateY(sin(radians(ang)) * 20);  rect(0, -50, 75, 100);
                    id: "righjoint",
                    position: [1, 0, .1],
                    scale: [0.5, 0.5, 0.5],
                    rotation: [0, -180, 0],
        ...
        skeleton.children.forEach((level1, idx) => {
            const geometry = new Box(gl, {width: level1.scale[0], height: level1.scale[1], depth: level1.scale[2]});
            const childLvl1 =  new Mesh(gl, {mode: gl[settings.rendering], geometry, program});
            ...
            level1.children.forEach(level2 => {
                console.log(level2)
                const geometry = new Box(gl, {width: level2.scale[0], height: level2.scale[1], depth: level2.scale[2]});
                const childLvl2 =  new Mesh(gl, {mode: gl[settings.rendering], geometry, program});
